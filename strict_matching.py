...
def calculate_match_score(row, spectra_row):
    ...
    # Mass matching (weight: 4) - must be very close (0.2g threshold)
    mass_diff = abs(row['Mass (g)'] - spectra_row['Mass (g)'])
    if mass_diff <= 0.2:  # Strict threshold for mass
        score += 4
    max_score += 4
    
    # Major diameter matching (weight: 2) - must be very close
    major_diff = abs(row['Major dia. (mm)'] - spectra_row['Major dia (mm)'])
    if major_diff <= 0.2:  # Strict tolerance
        score += 2
    elif major_diff <= 0.5:
        score += 1
    max_score += 2
    
    # Minor diameter matching (weight: 2) - must be very close
    minor_diff = abs(row['Minor dia. (mm)'] - spectra_row['Minor dia (mm)'])
    if minor_diff <= 0.2:  # Strict tolerance
        score += 2
    elif minor_diff <= 0.5:
        score += 1
    max_score += 2
...
def find_all_potential_matches(hsi_df, spectra_df):
    ...
    for _, hsi_row in hsi_df.iterrows():
        ...
        for _, spectra_row in spectra_df.iterrows():
            # Skip if mass difference is too large (>0.2g)
            if abs(hsi_row['Mass (g)'] - spectra_row['Mass (g)']) > 0.2:
                continue
                
            score = calculate_match_score(hsi_row, spectra_row)
            if score == 1.0:  # Only perfect matches
    ...
    # Sort potential matches by mass difference (smallest first)
    potential_matches.sort(key=lambda x: x['Mass_Diff'])
...
# Process potential matches in order of mass difference
...
final_matches_df.to_csv('one_to_one/strict_matches.csv', index=False)

# Print summary
print(f"Total HSI samples: {len(hsi_data)}")
print(f"Total Spectra samples: {len(spectra_data)}")
print(f"Total potential perfect matches found: {len(potential_matches)}")
print(f"Final one-to-one perfect matches: {len(final_matches_df)}")
...
print("\nFirst few matches:")
print(final_matches_df[['HSI_ID', 'Spectra_ID', 'HSI_Mass', 'Spectra_Mass', 'Mass_Difference']].head().to_string()) 